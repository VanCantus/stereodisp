...
__global__ void ssd(unsigned char *leftImg, unsigned char *rightImg, float *dispMap, 
...
__global__ void ncc(unsigned char *leftImg, unsigned char *rightImg, float *dispMap, 
		int width, int height, int frame, int delta_min, int delta_max, float ncc_min) {
	int xOff = blockIdx.x * blockDim.x + threadIdx.x;
	int yOff = blockIdx.y * blockDim.y + threadIdx.y;
...
__global__ void global_zmncc(unsigned char *leftNorm, unsigned char *rightNorm, float *dispMap, int width, 
...
__global__ void normalized_ssd(unsigned char *leftNorm, unsigned char *rightNorm, float *dispMap, int width, 
